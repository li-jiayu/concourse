...
`)
	...
	Expect(err).NotTo(HaveOccurred()) // TODO this was inspired by a silent panic in the server goroutine
}
...
func whenIArchiveIt(client concourse.Client, pipelineName string) {
	...
	Expect(err).ToNot(HaveOccurred()) // TODO this drove a change, but the panic would be a better diagnostic
}
...
func getPipeline(client concourse.Client, pipelineName string) atc.Pipeline {
	...
	Expect(err).ToNot(HaveOccurred()) // TODO this was inspired by a silent panic in the server goroutine
	return pipeline
}
